...
vi.mock("react-signature-canvas", () => {
  const MockSignatureCanvas = forwardRef(function MockSignatureCanvas(
    ...
    useImperativeHandle(ref, () => ({
      ...
      fromDataURL: (url: string) => setData(url || ""),
    }));
...
function preloadIntake(extra: Record<string, unknown> = {}) {
  return {
    preloadedState: {
      report: {
        reportData: null,
        reportDebugData: null,
        reportStatus: "not_generated" as const,
        reportGenAt: null,
        reportGenTime: null,
        selectedSystems: [],
        intakeForm: {
...
describe("IntakeCard signature pads", () => {
  ...
  it("blocks proceed when signatures are missing", async () => {
    let nextFn: (() => void | Promise<void>) | undefined;
    const registerNext = (fn: (() => void | Promise<void>) | null) => {
      nextFn = fn ?? undefined;
    };
    const nav = vi.fn();
